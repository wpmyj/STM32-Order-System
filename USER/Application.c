...
void RTC_Func(void)
{
	...
	Display_HZ(52,60,"年",16);
	Display_HZ(84,60,"月",16);
	Display_HZ(116,60,"日",16);
	Display_HZ(148,60,"时",16);
	Display_HZ(180,60,"分",16);
	Display_HZ(212,60,"秒",16);
	
	/*秒中断完成后更新显示*/
	if(sec_flag){
		Get_RTC_Time(RTC_CNT,&Disp_Time);
		LCD_ShowNum(20,60,Disp_Time.year,4,16);
		LCD_ShowNum(68,60,Disp_Time.month,2,16);
		LCD_ShowNum(100,60,Disp_Time.day,2,16);
		LCD_ShowNum(132,60,Disp_Time.hour,2,16);
		LCD_ShowNum(164,60,Disp_Time.min,2,16);
		LCD_ShowNum(196,60,Disp_Time.second,2,16);
	}
...
u8 *Input_Scan(void)
{
	...
	if((key==KEY_WKUP)&&WKUP_flag){
		...
		if(str_cmpx(user,"123456     ", 11)&&str_cmpx(passwd,"123456     ",11))
			KEY_LED = !KEY_LED;		
	}else if(WKUP_flag==0){
		WKUP_flag = 1;
		BACK_COLOR = BROWN;
