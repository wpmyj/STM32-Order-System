...
{	
	...
	WINDOWS_INIT_TYPE AddFood_Win={GBLUE,LBBLUE,94,"加菜","返回","选择"};
	/*界面初始化*/
	Windows_Init(AddFood_Win);
	/*显示菜单*/
	Windows_Titles(AddFood_Info,(u8 **)Menu.Food,LBBLUE);
	/*选择新的图标，添加高亮*/
	...
	do{
		...
		if(tmp1!=i||tmp2!=j){	
			...
			if(!Food_flag[tmp1][tmp2])
				Windows_Title(AddFood_Info,(u8 **)Menu.Food,tmp1,tmp2,LBBLUE);
			tmp1 = i;
			tmp2 = j;
			/*选择新的图标，添加高亮*/
			Windows_Title(AddFood_Info,(u8 **)Menu.Food,i,j,CYAN);
